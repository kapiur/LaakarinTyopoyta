...
export default function PikaohjeetPage() {
  ...
  // --- УЛУЧШЕННЫЙ ДВИЖОК ЛОГИКИ (AND/OR) ---
  const activeRuleHits = useMemo(() => {
    ...
    return [...soloHits, ...groupHits].map(r => ({
      ...r,
      severity: r.priority <= 20 ? "danger" : r.priority <= 40 ? "warning" : "info"
    }));
  }, [card, params]);
  ...
  const addConditionToRule = (groupId: string | null, baseRule: ClinicalRule) => {
    ...
    // Если это первое превращение в группу, обновляем существующее правило
    if (!baseRule.groupId) {
  ...
  return (
    <div className="max-w-[1600px] mx-auto h-[calc(100vh-100px)] flex flex-col gap-4 p-4 text-slate-900 font-sans relative animate-in fade-in duration-500">
      <header className="flex items-center justify-between bg-white px-8 py-4 rounded-[2rem] border shadow-sm flex-shrink-0">
        ...
        <div className="flex items-center gap-3">
          ...
          {card && <button onClick={startEditing} className="bg-blue-600 text-white px-5 py-2.5 rounded-xl font-bold hover:bg-blue-700 shadow-md transition-all active:scale-95 text-xs flex items-center gap-2 font-sans"><Edit2 size={14} /> Muokkaa</button>}
        </div>
      </header>
      ...
      <main className="flex-1 grid grid-cols-12 gap-6 min-h-0 overflow-hidden">
        ...
        <aside className="col-span-3 flex flex-col gap-4 min-h-0">
          <button onClick={() => {}} className="w-full py-3 bg-blue-50 text-blue-600 border border-blue-200 border-dashed rounded-2xl font-bold text-xs uppercase tracking-widest hover:bg-blue-100 transition-all flex items-center justify-center gap-2"><Plus size={16} /> Uusi pikaohje</button>
          <div className="relative">
            <Search className="absolute left-4 top-1/2 -translate-y-1/2 text-slate-400" size={16} />
            <input placeholder="Etsi..." className="w-full pl-11 pr-4 py-3 bg-white border border-slate-200 rounded-2xl outline-none focus:ring-2 focus:ring-blue-500/10 font-medium text-sm transition-all shadow-sm" value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} />
          </div>
            ...
            {loadingList ? <div className="flex justify-center py-10"><Loader2 className="animate-spin text-blue-500" /></div> : 
              cards.filter(c => c.title.toLowerCase().includes(searchTerm.toLowerCase())).map(c => (
                <button key={c.slug} onClick={() => setActiveSlug(c.slug)} className={classNames("w-full text-left p-4 rounded-2xl border transition-all relative group", activeSlug === c.slug ? "bg-blue-600 text-white border-blue-600 shadow-lg" : "bg-white border-slate-100 hover:border-blue-200")}>
                  <span className="font-bold text-sm truncate block uppercase tracking-tight">{c.title}</span>
        ...
        <section className="col-span-9 min-h-0 bg-white rounded-[2.5rem] border shadow-sm flex flex-col overflow-hidden">
           {loadingCard ? <div className="flex-1 flex items-center justify-center"><Loader2 className="animate-spin text-blue-500" /></div> : card ? (
             <div className="flex-1 grid grid-cols-12 min-h-0">
                {/* Inputs */}
                <div className="col-span-4 border-r overflow-y-auto p-8 space-y-6 custom-scrollbar">
                  <div className="flex items-center gap-2 mb-4"><Sparkles size={14} className="text-blue-500"/><span className="text-[10px] font-black text-slate-400 uppercase tracking-widest">Parametrit</span></div>
                  {card.fields?.sort((a,b)=>a.order-b.order).map(f => (
                    <div key={f.key} className="space-y-2">
                      ...
                      <div className="relative">
                        <input className="w-full p-4 bg-slate-50 border-none rounded-2xl outline-none focus:bg-white focus:ring-4 focus:ring-blue-500/5 font-bold transition-all shadow-inner" placeholder={f.placeholder || ""} value={params[f.key] || ""} onChange={e => setParams({...params, [f.key]: e.target.value})} />
                ...
                <div className="col-span-8 bg-slate-50/30 overflow-y-auto p-12 space-y-8 custom-scrollbar relative">
                   <header className="mb-10">
                      ...
                      <p className="text-slate-400 font-medium mt-1">{card.subtitle}</p>
                   </header>
                        ...
                        return (
                          <div key={s.key} className={classNames("p-8 rounded-[2rem] border transition-all duration-500 shadow-sm", hl?.severity === "danger" ? "bg-rose-50 border-rose-200" : hl?.severity === "warning" ? "bg-amber-50 border-amber-200" : hl?.severity === "info" ? "bg-blue-50 border-blue-200" : "bg-white border-slate-100")}>
                            <div className="flex justify-between items-start mb-4">
                              <h3 className={classNames("text-[10px] font-black uppercase tracking-widest", hl?.severity === "danger" ? "text-rose-800" : hl?.severity === "warning" ? "text-amber-800" : "text-slate-500")}>{s.title}</h3>
                              <div className="flex flex-col gap-1">{hl?.hints.map((h, i) => <div key={i} className="px-3 py-1 bg-white border border-slate-100 rounded-xl text-[9px] font-black flex items-center gap-2 shadow-sm animate-in slide-in-from-right-2"><AlertTriangle size={12} className="text-amber-500" /> {h}</div>)}</div>
                            </div>
                            <div className="prose prose-slate max-w-none text-slate-700 font-medium leading-relaxed"><ReactMarkdown>{s.content}</ReactMarkdown></div>
                          </div>
                        );
                      })}
                   </div>
      ...
      {isEditing && card && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-slate-900/60 backdrop-blur-sm p-6 animate-in fade-in">
          <div className="bg-white rounded-[3rem] shadow-2xl max-w-6xl w-full h-[90vh] flex flex-col overflow-hidden border">
            <header className="p-6 bg-slate-900 text-white flex justify-between items-center shrink-0">
               <div className="flex items-center gap-6">
                 <span className="font-bold text-xs uppercase tracking-widest flex items-center gap-3"><Settings size={18} className="text-blue-400"/> Hallinta</span>
            ...
            <div className="flex-1 overflow-y-auto p-10 custom-scrollbar space-y-10">
               ...
               {editTab === "rules" && (
                 <div className="space-y-6">
                   {Object.values(draft.rules.reduce((acc: any, r) => {
                     ...
                     <div key={gIdx} className="p-8 bg-blue-50/30 rounded-[2.5rem] border border-blue-100 space-y-4 relative shadow-sm animate-in slide-in-from-bottom-4">
                       ...
                       {group.map((r: ClinicalRule, rIdx: number) => (
                         <div key={rIdx} className="space-y-4">
                           ...
                           <div className="flex items-center gap-4 flex-wrap">
                             ...
                             <select className="p-3 bg-white border-none rounded-xl text-xs font-bold outline-none focus:ring-4 focus:ring-blue-500/5 min-w-[200px]" value={r.fieldKey} onChange={e => { r.fieldKey = e.target.value; setDraft({...draft}); }}>
                               {draft.fields.map(f => <option key={f.key} value={f.key}>{f.label}</option>)}
                             </select>
                             <select className="p-3 bg-white border-none rounded-xl text-xs font-bold outline-none focus:ring-4 focus:ring-blue-500/5" value={r.operator} onChange={e => { r.operator = e.target.value; setDraft({...draft}); }}>
                               <option value=">">&gt;</option><option value="<">&lt;</option><option value="==">==</option><option value="truthy">kyllä</option>
                             </select>
                             <input className="p-3 bg-white border-none rounded-xl text-xs font-bold outline-none focus:ring-4 focus:ring-blue-500/5 w-24" value={r.value} onChange={e => { r.value = e.target.value; setDraft({...draft}); }} />
                             {rIdx === 0 && (
                               <>
                                 <span className="text-[10px] font-black text-blue-600 uppercase tracking-widest">KOROSTA</span>
                                 <select className="p-3 bg-white border-none rounded-xl text-xs font-bold outline-none focus:ring-4 focus:ring-blue-500/5 min-w-[200px]" value={r.highlightSectionKey || ""} onChange={e => { group.forEach((rg: any) => rg.highlightSectionKey = e.target.value); setDraft({...draft}); }}>
                       ...
                       <div className="flex gap-4 pt-4 border-t border-blue-100 items-center justify-between">
                         <button onClick={() => addConditionToRule(group[0].groupId || null, group[0])} className="flex items-center gap-2 text-[10px] font-black text-blue-500 uppercase tracking-widest hover:text-blue-700 transition-colors"><Plus size={14}/> Lisää ehto (И)</button>
                         <div className="flex items-center gap-6">
                            <div className="w-48 space-y-2">
                               <label className="text-[9px] font-black text-slate-400 uppercase tracking-widest ml-2">Väri (Priority)</label>
                               <select className="w-full p-2.5 bg-white border-none rounded-xl text-[10px] font-black outline-none focus:ring-4 focus:ring-blue-500/5 uppercase" value={group[0].priority} onChange={e => { group.forEach((rg: any) => rg.priority = parseInt(e.target.value)); setDraft({...draft}); }}>
                       ...
                       <div className="space-y-2">
                          ...
                          <input className="w-full p-3.5 bg-white border-none rounded-xl text-sm font-medium outline-none focus:ring-4 focus:ring-blue-500/5 shadow-inner" value={group[0].addHint || ""} onChange={e => { group.forEach((rg: any) => rg.addHint = e.target.value); setDraft({...draft}); }} />
                       </div>
                     </div>
                   ))}
               ...
               {/* ОСТАЛЬНЫЕ ВКЛАДКИ (Content, Fields) остаются прежними */}
            </div>
