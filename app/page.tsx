...
export default function Dashboard() {
  ...
  const anonymize = (text: string) => text
    .replace(/\b\d{6}[-+ABCDEF]\d{3}[0-9A-Z]\b/gi, '[HETU]')
    .replace(/\b(04\d|050)\s?\d{3}\s?\d{3,4}\b/g, '[PUHELIN]');

  const clearTool = () => { setToolText(''); setToolResult(''); };
  ...
  const processToolText = async (selectedMode: string) => {
    ...
    setToolMode(selectedMode); setIsToolLoading(true);
    try {
      const response = await fetch('/api/chat', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ text: anonymize(toolText), mode: selectedMode }) });
      const data = await response.json();
      ...
      <div className="lg:col-span-2 space-y-6">
        ...
        <div className="bg-white rounded-2xl shadow-sm border border-slate-200 p-6 space-y-5">
          ...
          <div className="space-y-4"><textarea value={toolText} onChange={(e) => setToolText(e.target.value)} placeholder={t('dashboard.textAreaPlaceholder')} className="w-full h-40 p-4 text-sm border border-slate-200 rounded-2xl focus:ring-2 focus:ring-blue-500/20 outline-none bg-slate-50/30 transition-all resize-none font-medium" />
            <div className="flex flex-wrap gap-2 p-1 bg-slate-100 rounded-xl w-fit">{aiTools.map((btn) => <button key={btn.key} onClick={() => processToolText(btn.key)} disabled={isToolLoading || !toolText.trim()} title={btn.description} className={`flex items-center gap-2 px-5 py-2 rounded-lg text-xs font-bold transition-all disabled:cursor-not-allowed disabled:opacity-50 ${toolMode === btn.key ? 'bg-white text-blue-600 shadow-sm' : 'text-slate-500 hover:text-slate-700'}`}>{isToolLoading && toolMode === btn.key ? <Loader2 size={14} className="animate-spin" /> : aiToolIcons[btn.icon]}{btn.label.toUpperCase()}</button>)}</div>
