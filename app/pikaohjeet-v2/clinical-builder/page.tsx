...
export default function ClinicalBuilderPage() {
  ...
  function updateDraftSection(index: number, patch: Partial<Section>) { setDraft((current) => { if (!current) return current; const sections = [...current.sections]; sections[index] = { ...sections[index], ...patch }; return { ...current, sections: normalizeSectionOrders(sections) }; }); }
  ...
  async function saveDraft() { if (!draft) return; setSaving(true); try { const data = await postJson("/api/pikaohjeet-v2", { ...draft, sections: normalizeSectionOrders(draft.sections) }); setSavedSlug(data.slug); } catch (error: any) { alert(error?.message || "Tallennusvirhe"); } finally { setSaving(false); } }
    ...
    <div className="mx-auto flex min-h-[calc(100vh-96px)] max-w-[1700px] flex-col gap-4 p-4 text-slate-900">
      ...
      <main className="grid flex-1 grid-cols-12 gap-5">
        ...
        <section className="col-span-7 rounded-[2rem] border border-slate-100 bg-slate-50 p-6 shadow-sm">
          {!draft ? <div className="flex h-full min-h-[640px] items-center justify-center text-sm font-black uppercase tracking-widest text-slate-300">{dict.noDraft}</div> : <div className="space-y-5"><div className="rounded-[2rem] border border-slate-100 bg-white p-6 shadow-sm"><div className="mb-3 flex flex-wrap gap-2"><span className="rounded-xl border border-blue-100 bg-blue-50 px-3 py-1.5 text-[10px] font-black uppercase tracking-wide text-blue-700">CLINICAL</span><span className="rounded-xl border border-amber-100 bg-amber-50 px-3 py-1.5 text-[10px] font-black uppercase tracking-wide text-amber-800">{draft.sourceStatus}</span>{draft.meta?.processingMode && <span className="rounded-xl border border-slate-100 bg-white px-3 py-1.5 text-[10px] font-black uppercase tracking-wide text-slate-500">{String(draft.meta.processingMode)}</span>}</div><h2 className="text-2xl font-black tracking-tight text-slate-900">{dict.editDraft}</h2></div>{draft.warnings?.length ? <div className="rounded-[1.5rem] border border-amber-100 bg-amber-50 p-5 text-sm font-semibold text-amber-900"><div className="mb-2 flex items-center gap-2 text-xs font-black uppercase tracking-wide"><AlertTriangle size={16} /> {dict.warning}</div><ul className="list-disc space-y-1 pl-5">{draft.warnings.map((w, i) => <li key={i}>{w}</li>)}</ul></div> : null}<div className="grid gap-4 rounded-[2rem] border border-slate-100 bg-white p-5"><label className="text-[10px] font-black uppercase tracking-widest text-slate-400">{dict.titleLabel}</label><input value={draft.title} onChange={(e) => setDraft({ ...draft, title: e.target.value })} className="rounded-2xl border border-slate-200 bg-slate-50 p-4 text-sm font-bold outline-none" /><label className="text-[10px] font-black uppercase tracking-widest text-slate-400">{dict.description}</label><input value={draft.description || ""} onChange={(e) => setDraft({ ...draft, description: e.target.value })} className="rounded-2xl border border-slate-200 bg-slate-50 p-4 text-sm font-bold outline-none" /><label className="text-[10px] font-black uppercase tracking-widest text-slate-400">{dict.tags}</label><input value={(draft.tags || []).join(", ")} onChange={(e) => setDraft({ ...draft, tags: e.target.value.split(",").map((tag) => tag.trim()).filter(Boolean) })} className="rounded-2xl border border-slate-200 bg-slate-50 p-4 text-sm font-bold outline-none" /></div>{[...(draft.sections || [])].sort((a,b) => a.order - b.order).map((section, index) => <article key={section.key} className={`rounded-[2rem] border p-6 shadow-sm ${sectionTone(section.kind)}`}><div className="mb-4 flex flex-wrap items-center justify-between gap-3"><label className="text-[10px] font-black uppercase tracking-widest text-slate-500">{dict.sectionTitle} · {index + 1}</label><div className="flex flex-wrap gap-2"><button onClick={() => moveSection(index, -1)} disabled={index === 0} className="inline-flex items-center gap-1 rounded-xl border border-slate-200 bg-white px-3 py-2 text-[10px] font-black uppercase tracking-wide text-slate-600 disabled:opacity-40"><ArrowUp size={12} /> {dict.moveUp}</button><button onClick={() => moveSection(index, 1)} disabled={index === draft.sections.length - 1} className="inline-flex items-center gap-1 rounded-xl border border-slate-200 bg-white px-3 py-2 text-[10px] font-black uppercase tracking-wide text-slate-600 disabled:opacity-40"><ArrowDown size={12} /> {dict.moveDown}</button><button onClick={() => addSectionAfter(index)} className="inline-flex items-center gap-1 rounded-xl border border-blue-100 bg-blue-50 px-3 py-2 text-[10px] font-black uppercase tracking-wide text-blue-700"><Plus size={12} /> {dict.addBelow}</button><button onClick={() => removeSection(index)} disabled={draft.sections.length <= 1} className="inline-flex items-center gap-1 rounded-xl border border-rose-100 bg-rose-50 px-3 py-2 text-[10px] font-black uppercase tracking-wide text-rose-700 disabled:opacity-40"><X size={12} /> {dict.removeSection}</button></div></div><input value={section.title} onChange={(e) => updateDraftSection(index, { title: e.target.value })} className="mb-4 w-full rounded-2xl border border-slate-200 bg-white p-4 text-sm font-bold outline-none" /><label className="mb-2 block text-[10px] font-black uppercase tracking-widest text-slate-500">{dict.sectionContent}</label><textarea value={section.content} onChange={(e) => updateDraftSection(index, { content: e.target.value })} className="min-h-[210px] w-full rounded-[1.5rem] border border-slate-200 bg-white p-5 text-sm font-semibold leading-relaxed outline-none" /><div className="mt-4 rounded-[1.5rem] border border-slate-100 bg-white p-5"><h3 className="mb-3 text-xs font-black uppercase tracking-wide text-slate-500">{dict.preview}</h3><div className="prose prose-slate max-w-none text-sm font-semibold leading-relaxed"><ReactMarkdown>{section.content}</ReactMarkdown></div></div></article>)}<button onClick={saveDraft} disabled={saving} className="flex w-full items-center justify-center gap-2 rounded-2xl bg-emerald-600 px-5 py-4 text-xs font-black uppercase tracking-wide text-white shadow-lg shadow-emerald-100 hover:bg-emerald-700 disabled:opacity-50">{saving ? <Loader2 className="animate-spin" size={16} /> : <Save size={16} />} {dict.save}</button>{savedSlug && <a href="/pikaohjeet-v2" className="flex items-center justify-center gap-2 rounded-2xl border border-emerald-100 bg-emerald-50 px-5 py-4 text-xs font-black uppercase tracking-wide text-emerald-700"><CheckCircle2 size={16} /> {dict.saved}: {savedSlug}</a>}</div>}
        </section>
      </main>
    </div>
  );
