...
import { ArrowLeft, Clipboard, Loader2, Plus, Save, Search, Trash2, X } from "lucide-react";
...
const ui = {
  fi: { title: "Clinical card manager", subtitle: "Admin-only kliinisten pikaohjeiden muokkaus", back: "Takaisin Pikaohjeisiin", search: "Hae kliinisistä korteista...", select: "Valitse kliininen kortti", save: "Tallenna muutokset", archive: "Arkistoi", confirmArchive: "Arkistoidaanko tämä kliininen kortti?", titleLabel: "Otsikko", description: "Kuvaus", tags: "Tagit", status: "Tarkistusstatus", sourceStatus: "Lähdestatus", environment: "Ympäristö", audience: "Kohderyhmä", addSection: "Lisää osio", removeSection: "Poista osio", sectionTitle: "Osion otsikko", sectionContent: "Sisältö", saved: "Tallennettu", empty: "Ei kliinisiä kortteja", copied: "Kopioitu", copy: "Kopioi" },
  ru: { title: "Clinical card manager", subtitle: "Admin-only редактирование клинических карточек", back: "Назад в Pikaohjeet", search: "Поиск по клиническим карточкам...", select: "Выберите клиническую карточку", save: "Сохранить изменения", archive: "В архив", confirmArchive: "Отправить эту клиническую карточку в архив?", titleLabel: "Заголовок", description: "Описание", tags: "Теги", status: "Статус проверки", sourceStatus: "Статус источников", environment: "Среда", audience: "Аудитория", addSection: "Добавить секцию", removeSection: "Удалить секцию", sectionTitle: "Заголовок секции", sectionContent: "Содержание", saved: "Сохранено", empty: "Клинических карточек нет", copied: "Скопировано", copy: "Копировать" },
  en: { title: "Clinical card manager", subtitle: "Admin-only clinical card editing", back: "Back to Pikaohjeet", search: "Search clinical cards...", select: "Select a clinical card", save: "Save changes", archive: "Archive", confirmArchive: "Archive this clinical card?", titleLabel: "Title", description: "Description", tags: "Tags", status: "Review status", sourceStatus: "Source status", environment: "Environment", audience: "Audience", addSection: "Add section", removeSection: "Remove section", sectionTitle: "Section title", sectionContent: "Content", saved: "Saved", empty: "No clinical cards", copied: "Copied", copy: "Copy" },
};

const statuses = ["NEEDS_REVIEW", "KAYPA_HOITO_CHECKED", "LOCAL_GUIDE", "PARTLY_CHECKED"];
const sourceStatuses = ["NOT_CHECKED", "NEEDS_REVIEW", "KAYPA_HOITO_CHECKED", "LOCAL_SOURCE", "PARTLY_CHECKED"];
...
export default function ClinicalManagerPage() {
  ...
  const filteredCards = useMemo(() => { const q = query.trim().toLowerCase(); if (!q) return cards; return cards.filter((card) => [card.title, card.description || "", ...(card.tags || [])].join(" ").toLowerCase().includes(q)); }, [cards, query]);

  function updateSection(index: number, patch: Partial<Section>) { setDraft((current) => { if (!current) return current; const sections = [...current.sections]; sections[index] = { ...sections[index], ...patch }; return { ...current, sections }; }); }
  function addSection() { setDraft((current) => current ? { ...current, sections: [...current.sections, makeSection(current.sections.length)] } : current); }
  function removeSection(index: number) { setDraft((current) => { if (!current || current.sections.length <= 1) return current; return { ...current, sections: current.sections.filter((_, idx) => idx !== index) }; }); }
