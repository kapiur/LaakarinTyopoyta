...
type LegacyCalculatorsPageProps = {
  initialTab?: string;
};

export default function CalculatorsPage({ initialTab = 'pca' }: LegacyCalculatorsPageProps) {
  const allowedTabs = new Set(['pca', 'vte', 'pe', 'cad', 'chads', 'peds', 'bmi', 'gfr']);
  const resolvedInitialTab = allowedTabs.has(initialTab) ? initialTab : 'pca';
  ...
  const toggleFactor = (key: string) => {
    setCad(prev => ({
      ...prev,
      factors: { ...prev.factors, [key as keyof typeof prev.factors]: !prev.factors[key as keyof typeof prev.factors] }
    }));
  };
  ...
  useEffect(() => {
    const nextTab = allowedTabs.has(initialTab) ? initialTab : 'pca';
          ...
          <div className="flex-1 overflow-y-auto no-scrollbar space-y-6 pr-1">
              ...
              <div className="space-y-5 animate-in fade-in">
                <div className="grid grid-cols-2 gap-3 p-4 bg-slate-50 rounded-3xl border border-slate-100">
                  <div className="space-y-1">
                    ...
                    <input type="number" value={peds.weight} onChange={e => setPeds({...peds, weight: e.target.value})} className="w-full p-4 bg-white border-2 border-transparent focus:border-blue-500 rounded-2xl font-bold outline-none" />
                  </div>
                  <div className="space-y-1">
                    <label className="text-[10px] font-extrabold text-blue-600 uppercase ml-1">Annos (mg/kg/vrk)</label>
                    <input type="number" value={peds.doseMgKg} onChange={e => setPeds({...peds, doseMgKg: e.target.value})} className="w-full p-4 bg-white border-2 border-transparent focus:border-blue-500 rounded-2xl font-bold outline-none" />
                  </div>
                </div>
                <div className="space-y-1">
                  <label className="text-[10px] font-extrabold text-blue-600 uppercase ml-1">Kuurin kesto (päivää)</label>
                  <input type="number" value={peds.days} onChange={e => setPeds({...peds, days: e.target.value})} className="w-full p-4 bg-slate-50 border-2 border-transparent focus:border-blue-500 focus:bg-white rounded-2xl font-bold outline-none" />
                </div>
                <div className="space-y-1">
                  <label className="text-[10px] font-extrabold text-blue-600 uppercase ml-1">Vahvuus (mg / ml)</label>
                  <input type="number" value={peds.strength} onChange={e => setPeds({...peds, strength: e.target.value})} className="w-full p-4 bg-slate-50 border-2 border-transparent focus:border-blue-500 focus:bg-white rounded-2xl font-bold outline-none" />
                </div>
                  ...
                  <div className="grid grid-cols-6 gap-2">
                    {[1,2,3,4,5,6].map(n => (
                      <button key={n} onClick={() => setPeds({...peds, timesPerDay: n})} 
                        className={`py-3 rounded-xl font-black text-xs transition-all border ${peds.timesPerDay === n ? 'bg-blue-600 text-white border-blue-600 shadow-md' : 'bg-slate-50 text-slate-400 border-transparent'}`}>
                        {n}x
                      </button>
                    ))}
                  </div>
                ...
                <div className="space-y-1">
                  ...
                  <input type="number" value={peds.bottleSize} onChange={e => setPeds({...peds, bottleSize: e.target.value})} className="w-full p-4 bg-slate-50 border-2 border-transparent focus:border-blue-500 focus:bg-white rounded-2xl font-bold outline-none" />
                </div>
                <button onClick={resetPeds} className="w-full py-4 border-2 border-slate-100 rounded-2xl text-[11px] font-black uppercase text-slate-400 hover:text-red-500 transition-all flex items-center justify-center gap-2">
            ...
            {activeTab === 'pca' && (
              <div className="space-y-4 animate-in fade-in">
                ...
                <div className="mt-6 p-5 bg-slate-50 rounded-[2rem] border border-slate-100 shadow-sm">
            ...
            {activeTab === 'vte' && (
              <div className="space-y-2 animate-in fade-in">
                {[
                  ...
                  {l: "Aiempi ТГВ", k: "prev", v: 1}, {l: "Muu dg todennäköisempi", k: "alt", v: -2},
                ].map(item => (
        ...
        <div className="bg-white rounded-[2.5rem] p-6 sm:p-8 shadow-sm flex flex-col border border-slate-200 min-h-[600px]">
          <div className="flex justify-between items-center mb-6">
            ...
            {(result || (activeTab === 'peds' && pedsResult.dailyMg > 0)) && (
              <button onClick={handleCopy} className={`px-4 py-2 rounded-xl font-bold text-[10px] transition-all flex items-center gap-2 shadow-sm ${copied ? 'bg-emerald-500 text-white shadow-md' : 'bg-slate-100 text-slate-600 hover:bg-slate-200'}`}>
            ...
            {activeTab === 'peds' ? (
              <div className="space-y-6 animate-in fade-in">
                ...
                {pedsResult.totalMl > 0 && (
                  <div className="p-7 bg-blue-600 rounded-[2.5rem] text-white shadow-2xl shadow-blue-200 border-4 border-blue-500">
                ...
                <div className="animate-in fade-in duration-200 space-y-6">
                  {result.type === 'vte_pe_result' && (
                    <div className="space-y-6">
                      ...
                      <div className="bg-slate-50 p-8 rounded-[2.5rem] border border-slate-100 italic font-bold leading-relaxed text-lg">"{result.rec}"</div>
                    </div>
                  )}

                  {result.type === 'cad_result' && (
                    <div className="space-y-6">
                      <div className={`p-8 rounded-[3rem] text-white shadow-xl ${result.color === 'blue' ? 'bg-blue-500' : 'bg-amber-500'}`}>
                        <p className="text-[10px] font-bold uppercase tracking-widest text-center">Ennakkotodennäköisyys</p>
                        <div className="text-8xl font-black text-center">{result.prob}<span className="text-3xl">%</span></div>
                      </div>
                    </div>
                  )}
                  ...
                  {result.type === 'single' && (
                    <div className="text-center bg-slate-50 p-12 rounded-[3rem] border border-slate-100">
                      ...
                      <div className="text-8xl font-black text-blue-600 mb-2">{result.score}</div>
                  ...
                  {result.type === 'dual' && (
                    <div className="space-y-4">
                      ...
                      <div className="bg-slate-900 p-6 rounded-[2rem] text-center text-white font-bold">{result.desc}</div>
                    </div>
                  )}
                </div>
              )
